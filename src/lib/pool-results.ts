...
export async function computePoolResults(poolId: string) {
  ...
  const aggregated = new Map<
    string,
    { key: string; name: string; totalPercent: number; sessionCount: number; isFixedShare: boolean }
  >()
  ...
  const results: AggregatedResult[] = Array.from(aggregated.values())
    .map((r) => ({
      key: r.key,
      name: r.name,
  ...
  return {
    pool: { id: pool.id, name: pool.name, status: pool.status, companyId: pool.companyId },
    results,
